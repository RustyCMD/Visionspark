...
ADD COLUMN IF NOT EXISTS enhancement_limit INT NULL;

COMMENT ON COLUMN public.profiles.current_subscription_tier IS 'Active subscription tier, e.g., monthly_unlimited. Set by validation function.';
...
COMMENT ON COLUMN public.profiles.enhancement_limit IS 'User-specific base daily enhancement limit if not subscribed, or a custom limit. Can be overridden by active subscription tier benefits.';
